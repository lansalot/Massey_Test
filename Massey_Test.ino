...
byte mfRollingCounter[8] = { 0, 0, 0, 0, 0, 0, 0, 0 }; // MF8S appears to have a rolling counter in effect
  // let's take note of this in case we get ignored for being out of sequence or something
  ...
  void loop()
  {
    ...
    if (currentTime - lastTime >= LOOP_TIME)
    {
      ...
      if (steeringValveReady != 20 && steeringValveReady != 16)
      {
        ...
        if (Brand == 1) SetRelaysMassey();  //If Brand = Massey run the hitch control bottom of this page
  ...
  if (udpData[0] == 0x80 && udpData[1] == 0x81 && udpData[2] == 0x7F) //Data
  {
    if (udpData[3] == 0xFE)  //254
    {
	  ...
	  if ((bitRead(guidanceStatus, 0) == 0) || (steerSwitch == 1))        //AgOpen or Steer switch off
	  {
          ...
          if (intendToSteer == 1)
          {
              if ((bitRead(guidanceStatus, 0) == 0)) Serial.println("GuidanceStatus = Off");
              if (steerSwitch == 1) Serial.println("SteerSwitch = Off");
          }
	  }
...
//Only if tool lift is enabled AgOpen will press headland buttions via CAN
if (aogConfig.enableToolLift == 1){
  if (bitState  && !bitStateOld) PressMasseyF1(); //Press F1 button on joystick
  if (!bitState && bitStateOld) PressMasseyF2(); //Press F2 button on joystick
}
