...
void K_Receive()
{
	...
	if (K_Bus.read(KBusReceiveData)) {
		...
		if (Brand == 1)
		{
			...
			{
				// copy VBusReceiveData.buf to mfRollingCounter in case we want to send an F1/F2
				memcpy(mfRollingCounter, KBusReceiveData.buf, 8);
...
// Massey function buttons
void PressMasseyF1()
{
	CAN_message_t buttonData;
	buttonData.id = 0xCFF2621;
	buttonData.flags.extended = true;
	buttonData.len = 8;
